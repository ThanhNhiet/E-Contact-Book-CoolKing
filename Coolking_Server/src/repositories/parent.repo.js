...
const getParentByParent_id = async (parent_id) => {
  try {
    ...
    const result = await models.Parent.findOne({
      attributes: {
        exclude: ['id', 'isDeleted', 'createdAt', 'updatedAt']  // Remove student_id from exclude
      },
    ...
    if (!result.student_id) {
      throw new Error(`No student associated with parent ID: ${parent_id}`);
    }
    ...
    if (!student) {
      throw new Error(`Student not found with ID: ${result.student_id}`);
    }
    ...
    const genderParent = result.gender === "1" ? "Nam" : "Nữ";
    return {
